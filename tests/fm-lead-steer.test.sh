...
# These tests drive the real fm-lead over the real fm-send and a stubbed
# tmux whose inventory and pane command decide who is alive.
set -u
...
test_trim_orders_then_types_compact() {
  ...
  [ ! -d "$home/state/c1.inbox" ] || fail "a /compact is never an inbox record (the harness's parser must see it)"
  idx="$home/data/c1/trims/index"
  [ -f "$idx" ] || fail "the order is recorded in the crewmate's trims index"
  case "$(cut -f1,3,4 "$idx")" in
  ...
  [ "$(sed -n '2p' "$idx" | cut -f4)" = '-' ] || fail "a bare order records focus '-'"
