...
FM_LEADER_ESCALATE_SECS=1800       # seconds a led crewmate's keyed door may stay open under its live leader before the watcher wakes First Mate for it once (bin/fm-watch.sh, the chain section)
FM_SIGNAL_CHECK_SECS=300           # seconds between the watcher's transcript-signal checks of one led crewmate (bin/fm-crew-signals.sh, run from the poll; the leader is rung, never First Mate)
FM_STUCK_CALL_SECS=900             # seconds a led crewmate may be busy with nothing new in its transcript before its leader is rung once for a stall
FM_DRIFT_TOKENS=40000              # tokens spent since a led crewmate's last commit, with no logbook change over that spend, before its leader is rung once with a drift candidate
FM_LEAD_ORDER_STALE_SECS=430       # seconds an ordered trim may stand unanswered in a led crewmate's trim ledger before its leader is rung once; twice the longest compaction measured on this fleet (215 s; docs/branch-leader.md), so an ordinary slow trim is never called stale
