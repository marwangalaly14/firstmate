...
FM_LEAD_TRIM_WAIT_SECS=600         # seconds bin/fm-lead.sh trim waits for the crewmate's own trim line before it gives up on the continue-steer, tells the leader and exits 3
FM_LEAD_TRIM_POLL_SECS=2           # seconds between that wait's reads of data/<crewmate>/trims/index
