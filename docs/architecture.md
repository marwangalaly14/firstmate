...
A claude worker's context compacts at a measured trigger instead of filling its whole window, the story spine comes back from disk after each compaction, and a worker that compacts a second time is told to the branch leader, never stopped.
`bin/fm-compact-lib.sh` owns the measured trigger law, the budget parser, and the decision; `bin/fm-spawn.sh` writes the measured `autoCompactWindow` into the worker worktree's `.claude/settings.local.json` (project-local settings beat user settings, so captain and primary sessions are untouched), wires the SessionStart hook, and writes the spine's gate key as the task record's `budget=` field; `bin/fm-compact-spine.sh` is the SessionStart compact hook that appends the `compacted <n> at <peak>` line to the story's own `state/<id>.compactions` ledger (never to the status log, which is the wake channel), writes the one-compaction incident line to the home's learnings, enqueues one `signal` wake for the branch leader at a second compaction while the worker carries straight on, and prints the Definition of done, budget, ledger, status tail, report, and branch commits from disk so the compaction summary is never trusted with anything that matters.
The count is the story's rather than the session's: the larger of the transcript's own automatic `compact_boundary` rows (true even for compactions that ran before the machinery existed; a hand-run `/compact` still fires the hook and still earns the spine reprint, but is deliberate tidying rather than a head that filled, so the harness's own `compactMetadata.trigger` keeps it out of the count) and the highest number the ledger already recorded (true across a relaunch, whose fresh transcript has no boundaries at all, and monotonic - the count never drops below what the machinery itself wrote down), with the ledger alone when the transcript is missing or unparseable; the count and the peak come from a single pass over the transcript, and the peak is read from after its last boundary so a prior head's numbers are never re-reported. The durable incident line and the branch leader's signal name a budget only when a human wrote a `Budget:` line into the brief: a worker compacts AT the machine trigger, so measuring its peak against that number says nothing.
The gate opens on the spawn-written `budget=` record, with a hand-written brief `Budget:` line as the human override; with neither, every part stays silent.
`tests/token-budget.test.sh` pins the decision, the parsers, the hook's output and side effects against briefs produced by the real scaffold, and the spawn wiring.
