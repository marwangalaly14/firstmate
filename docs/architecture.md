...
The count is the story's rather than the session's: the larger of the transcript's own `compact_boundary` rows (true even for compactions that ran before the machinery existed) and the ledger's recorded fires (true across a relaunch, whose fresh transcript has no boundaries at all), with the ledger alone when the transcript is missing or unparseable, and the peak is read from after the transcript's last boundary so a prior head's numbers are never re-reported.
