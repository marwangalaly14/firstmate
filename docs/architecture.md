...
The count comes from the transcript's own `compact_boundary` rows (true even for compactions that ran before the machinery existed) with the ledger as fallback, and the peak is read from after the transcript's last boundary so a prior head's numbers are never re-reported.
