...
A decision whose stated key that fold cannot accept is not dropped: the key is refused (it never becomes the shared `default` key, and it rings no door), but the decision still opens under a derived `unusable-key-...` key whose summary names the refused key ahead of the worker's own words, so a mistyped key costs a reader one confusing line instead of costing the captain a question they were never shown.
...
While a live branch leader holds a crewmate, that plane carries only the leader's own steer, the captain's quoted words, and lifecycle to it, and refuses every other text send before any durable write (`bin/fm-send.sh`'s header owns the led channel; [`branch-leader.md`](branch-leader.md) owns the leader's side of it).
