...
The count is the story's rather than the session's: the larger of the transcript's own `compact_boundary` rows (true even for compactions that ran before the machinery existed) and the highest number the ledger already recorded (true across a relaunch, whose fresh transcript has no boundaries at all, and monotonic - the count never drops below what the machinery itself wrote down), with the ledger alone when the transcript is missing or unparseable; the count and the peak come from a single pass over the transcript, and the peak is read from after its last boundary so a prior head's numbers are never re-reported. The durable incident line and the branch leader's signal name a budget only when a human wrote a `Budget:` line into the brief: a worker compacts AT the machine trigger, so measuring its peak against that number says nothing.
