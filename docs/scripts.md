...
| `fm-send.sh`             | Steer a task via a durable inbox record plus doorbell, or send a supported key or typed harness invocation through the recorded backend; to a led crewmate whose leader is alive, only `--from-leader`, `--captain` or `--lifecycle` sends get through (the led channel, recorded as `mark=` in the record) |
...
| `fm-crew-signals.sh`     | A led crewmate's transcript signals: reads its card and rings its leader once per episode of a stall (busy with nothing new for `FM_STUCK_CALL_SECS`), a loop (the card's repeats) or a drift candidate (`FM_DRIFT_TOKENS` since the last commit with no logbook change over that spend), keeping `data/<id>/signals/index`; the ring carries the `--outside` card, so none of the crewmate's own words enter it; a dead leader is recorded once and never hammered; an unled crewmate never rings; run by `fm-watch.sh` once per `FM_SIGNAL_CHECK_SECS` per led crewmate, never waking First Mate |
...
| `fm-trim-event.sh`       | Record every trim of a crewmate's context from a claude PostCompact hook (`data/<id>/trims/`) and, from the second automatic trim on, put one line into the leader's steering inbox or one signal wake before First Mate; after a trim a leader ordered, send the crewmate its carry-on nudge |
